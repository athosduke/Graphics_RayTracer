...
int main(int argc, char **argv)
{
	...
	glm::vec3 basis2 = -glm::normalize(glm::cross(dir, left));
		...
		if (raytracing)
		{
			...
			for (int y = 0; y < SCR_HEIGHT; y++)
			{
				...
				for (int x = 0; x < SCR_WIDTH; x++)
				{
					...
					glm::vec3 currentdir = lookAt - eye + float(x - (float)SCR_WIDTH/ 2 + 0.5f) * (width / SCR_WIDTH) * base1 + float(y - (float)SCR_HEIGHT / 2 + 0.5f) * (height / SCR_HEIGHT) * base2;
					//glm::vec3 currentDir, basis1, basis2;
					//float u, v;
					//u = (x - float(SCR_WIDTH / 2)) / SCR_HEIGHT;
					//v = -(y - float(SCR_HEIGHT / 2)) / SCR_HEIGHT;

					//currentDir = dir + u * (-left) + v * (up);

					//currentDir = dir + glm::operator*(float(x-SCR_WIDTH/2), -left) + glm::operator*(float(y - SCR_HEIGHT/2), -up);
