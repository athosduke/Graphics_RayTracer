...
glm::vec3 scene::rayTrace(glm::vec3 eye, glm::vec3 dir, int recurseDepth)
{
	...
	glm::vec3 normalsaved = normal;//check if normal stay the same
	...
	if ((matIndex == 1) && (recurseDepth == 0)) {

	}
	//add diffuse color times ambient light to our answer
	answer += ambLight * myMaterials[matIndex].diffuseCol;
	//iterate through all lights
	glm::vec3 intersectP = eye + (dist*dir);
	////if the light can see the surface point,
	//for (unsigned int i = 0; i < myLights.size(); i++) {
	//	glm::vec3 direction = glm::normalize(intersectP - myLights[i].position);

	//	float distance = myObjGroup->testIntersections(myLights[i].position, direction);
	//	if (dist < 9999999) {
	//		glm::vec3 n = normal;
	//		if (glm::dot(dir, n) < 0)
	//			n = -n;
	//		answer += myLights[i].color * myMaterials[matIndex].diffuseCol * glm::dot(direction, n);//diffuse

	//		glm::vec3 h = glm::normalize((dir + direction));
	//		answer += myLights[i].color * myMaterials[matIndex].specularCol * glm::pow(glm::dot(normal, h), myMaterials[matIndex].shininess);//specular
	//	}
	//}
	for (int i = 0; i < myLights.size(); i++) {
		glm::vec3 lightV = myLights[i].position - intersectP;
		lightV = glm::normalize(lightV);
		float intersectionDist = myObjGroup->testIntersections(myLights[i].position, -lightV);
		float distance = fabsf(glm::distance(myLights[i].position, intersectP));
		if ((intersectionDist < distance + 0.0001f) ||(intersectionDist > distance - 0.0001f) ){
			float diffuse = 0.0;
			if (glm::dot(normal, lightV) > 0) {
				diffuse = glm::dot(normal, lightV);
			}
			answer += (texture->diffuseCol*(myLights[i].color*diffuse)) + (myLights[i].color*texture->specularCol*(pow(glm::dot((glm::normalize(lightV + glm::normalize(eye - intersectP))), normal), texture->shininess)));
		}
	}
	...
	if (recurseDepth < 3)
	{
		glm::vec3 refractedRay, refractColor;
		recurseDepth++;
		if (texture->reflectiveCol != glm::vec3(0.0f)) {
			/*glm::vec3 camera_view = dir;
			camera_view = glm::normalize(camera_view);
			glm::vec3 normal_ref = normal;

			if (glm::dot(camera_view, normal) < 0.0f) {
			normal_ref = -normal;
			}
			glm::vec3 reflection_dir = camera_view - 2 * glm::dot(camera_view, normal_ref)*normal_ref;
			glm::vec3 ref_answer = rayTrace(surface, reflection_dir, recurseDepth);*/
			glm::vec3 reflect = glm::normalize(dir - (2 * (glm::dot(dir, normal)) * normal));
			glm::vec3 newpoint = (glm::dot(reflect, normal) < 0) ? (intersectP - normal) : (intersectP + normal);
			answer += (texture->reflectiveCol * rayTrace(newpoint, reflect, recurseDepth));
			//answer += ref_answer * texture->reflectiveCol;
		}
		...
		if (texture->transparentCol != glm::vec3(0))
		{
			if (glm::dot(normal, dir) < 0){
				float pheta = acos((glm::dot(normal, (-dir))));
				float angle = asin((texture->refractionIndex)*sin(pheta));

				refractedRay = ((dir + (normal*cos(pheta))) * (texture->refractionIndex)) - (normal * cos(angle));
				refractedRay = glm::normalize(refractedRay);

				
				refractColor = rayTrace(intersectP, refractedRay, recurseDepth);
			}
			else //else, bend away
			{
				float newangle = acos((glm::dot((-normal), (-dir))));
				float newpheta = asin((texture->refractionIndex)*sin(newangle));

				refractedRay = (((dir + ((-normal)*cos(newangle)))) / (texture->refractionIndex)) - ((-normal) * cos(newpheta));
				refractedRay = glm::normalize(refractedRay);
